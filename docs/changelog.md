...
- **OPS-001 partial 2026-05-26** — Supabase project in `ap-south-1` (10 tables, RLS enabled, custom plpgsql `uuidv7()` implementing RFC 9562 — `pg_uuidv7` extension not available in region, R-OPS-2 resolved), Vercel project `aura-web` with function region `bom1` (Turborepo monorepo pattern at repo root), Vercel AI Gateway with Anthropic + OpenAI providers + Zero Data Retention. Sentry / Bhashini (→ Sarvam post-amendment) / MSG91 deferred with DRI Decisions.
- **AUR-5 end-to-end smoke validated 2026-05-26** — on-device walkthrough on iOS via Expo Go against live Vercel + Supabase ap-south-1. AC1, AC2, AC3, AC7, AC8 (partial), AC11 confirmed empirically. Full backend works in production (curl test of `auth.passkey.beginEnrollment` returns HTTP 200 with valid options + signed challenge token).
...
- **Architecture amendment 2026-05-26 — speech provider Bhashini → Sarvam AI** (SUPERSEDES Decision). Same model lineage (AI4Bharat) on production SaaS infrastructure; AI4Bharat OSS retained as self-hosted disaster-recovery fallback. R-SPEECH amended + new R-SPEECH-2 (vendor concentration + cost-floor) logged.
- **Expo SDK 52 dependency alignment (apps/mobile)** — pinned React + react-dom to `18.3.1` (Expo SDK 52's required version, was `^19.0.0`), pinned `react-native-safe-area-context` to `4.12.0`, pinned `react-native-screens` to `~4.4.0`, pinned `expo-secure-store` + `expo-av` to SDK 52 ranges, added missing peer deps (`expo-constants`, `expo-linking`, `@expo/metro-runtime`, `@babel/runtime`), enabled New Architecture in `app.config.ts`.
- **Repo-root `.npmrc` with `node-linker=hoisted`** + public-hoist patterns for Expo / RN / Babel / @expo packages. Required for pnpm to play well with Metro's resolution model in a monorepo (community-standard fix).
- **Mobile `metro.config.js`** added with workspace-monorepo support: package-exports field resolution (modern ESM packages like superjson → copy-anything), workspace-root watch folder, single-React resolution via `extraNodeModules`.
