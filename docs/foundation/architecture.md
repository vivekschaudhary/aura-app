...
| **Cost optimization** | All-in cost per WAR per month (Vercel + Supabase + AI Gateway + EAS + Sarvam AI + MSG91 OTP fallback) | **≤ ₹20 / WAR / month at 1,000+ WAR sustained.** Year-1 infra budget ceiling: ₹20 × 100,000 WAR × 12 mo = **₹2.4 crore** (worst case at year-end WAR target). | Direct from product v2 § Guardrails — non-negotiable in the free-in-v1 model since there's no revenue to absorb a miss |
...
| Speech (ASR / TTS) | **Sarvam AI** (Hindi + Indian-English ASR + TTS); self-hosted AI4Bharat IndicASR / IndicTTS as disaster-recovery fallback wired at module boundary. _Superseded the 2026-05-24 Bhashini-primary decision; see DRI Decision 2026-05-26._ | easy |
...
#### Speech: Sarvam AI (with self-hosted AI4Bharat OSS as DR fallback) — _amended 2026-05-26_

| Pillar | Score | Rationale | Research citation |
|--------|-------|-----------|-------------------|
| Reliability | acceptable | Private SaaS with documented uptime + responsive operator support. DR path: self-host AI4Bharat OSS models if Sarvam suffers a sustained outage (≥1 hr to bring up; degraded operational state). Concentration risk tracked in R-SPEECH-2. | research §3E, §5F |
| Security | good | India-region inference (DPDPA preserved). OSS model lineage available for self-host fallback. | research §3E, §5F |
| Performance efficiency | acceptable | Hindi + Indian-English code-switching reportedly handled better than Bhashini-platform alternatives; per-language quality eval (R-SPEECH) gates AUR-2 brief promotion. | research §2D, §5F |
| Cost optimization | acceptable | Single-digit rupees per 5-min session at public pricing; fits ≤₹20/WAR/mo ceiling with headroom; no longer the "negligible" line item the Bhashini decision assumed. Tracked in R-SPEECH-2. | research §2D, §5F |
| Operational excellence | good | Private SaaS with onboarding flow + docs + support (vs Bhashini-platform's gov-tech operator gap); single vendor to monitor. | research §5F |
| Sustainability | acceptable | Well-funded startup (Lightspeed-led Series A 2024); not state-backed long-horizon guarantee. DR path on OSS models is durable independent of Sarvam's corporate trajectory. | research §3E |
...
|--------|----------------------|------------------|--------------|
| **Chosen: TS monorepo, Next.js + Expo, Supabase, AI Gateway (Claude + OpenAI), Sarvam AI, passkey-primary auth** _(speech provider amended 2026-05-26 — was Bhashini)_ | Meets all 6 fitness thresholds with headroom (esp. pgvector P95 has 15× headroom over the 1.5s target). Data model fits: relational + RLS + single vector index. | All 6 pillars `acceptable` or better; zero `poor` cells | — |
...
- [2026-05-26] [Engineer / Vivek] **SUPERSEDES the 2026-05-24 Bhashini-primary decision below. Speech provider: Sarvam AI (primary) + self-hosted AI4Bharat IndicASR/IndicTTS (DR fallback).**
  - **Rationale (required):** The original Bhashini decision conflated two layers: the *model quality* (AI4Bharat's IndicASR/IndicTTS — academically strong) and the *platform reliability* (Bhashini = the MeitY ULCA gov-platform layer wrapping those models — reported operational quality is patchy, registration cold-start is slow, inference uptime variable). Sarvam AI (private company spun out of AI4Bharat alumni) ships effectively the same model lineage on production SaaS infrastructure: India-region inference (DPDPA-residency preserved), responsive operator support, documented uptime, and reportedly stronger Hindi-English code-switching handling. We retain the open-source AI4Bharat models as a self-hosted disaster-recovery path if Sarvam suffers a sustained outage — same models, different operator, decouples vendor-concentration risk. Cost moves from "negligible (Bhashini free tier)" to "single-digit rupees per session" but still fits comfortably under the ≤₹20/WAR/mo Cost fitness ceiling (per the cost analysis in R-SPEECH-2 below).
  - **Area (required, tag):** architecture / AI / speech / reliability.
  - **Alternatives considered (required):** Stay on Bhashini-primary (rejected: platform-reliability skepticism from Vivek 2026-05-26 — "just because it exists doesn't mean it works in Indian govt"); Whisper for ASR + ElevenLabs for TTS (rejected: DPDPA residency lost — neither hosts in India); self-host AI4Bharat models on Modal/Fly from Day 1 (rejected: fails Op-ex fitness at current team size; deferred to future architectural-initiative bet); evaluate-then-decide (rejected as deferral pattern: amendment locks the direction now, AUR-2 brief promotion still gates the empirical eval that confirms quality bar — see R-SPEECH below as amended).
  - **Reversibility:** easy. The `packages/ai/src/speech.ts` boundary is unchanged; only the provider wiring inside it swaps. The DR fallback (self-hosted AI4Bharat) is a real production-shippable Plan-B if Sarvam goes away.

- [2026-05-24] [Enterprise/Solution Architect] ~~Bhashini / AI4Bharat for vernacular ASR/TTS; Sarvam fallback wired at module boundary.~~ **SUPERSEDED 2026-05-26 — see Decision above.** Preserving for traceability.
...
- [2026-05-24] [Enterprise/Solution Architect] R-SPEECH: **Sarvam AI** production quality at our conversational workload not empirically validated for our v1 languages (English + Hindi). Ramp languages (Tamil, Telugu, Bengali, Marathi, Kannada) blocked until per-language quality eval passes. _Amended 2026-05-26: subject pivoted from Bhashini to Sarvam following the SUPERSEDES decision above; risk shape unchanged._
  - **Likelihood (required):** medium for Hindi (Sarvam is built by AI4Bharat alumni — well-aligned with our user audio profile, but not empirically tested by us at production volume); low for English (multiple credible options: Sarvam, OpenAI Whisper, native iOS Speech / Android SpeechRecognizer).
  - **Impact (required):** high (voice is the primary UX).
  - **Mitigation (required):** (1) Pre-MVP quality eval for v1 launch languages — record 50–100 reflection-style voice samples in Hindi + 50 in English from target-persona-matched speakers (not synthetic), score WER for ASR + MOS for TTS. (2) For English, evaluate Sarvam vs Whisper vs native; pick the best quality-cost trade with DPDPA preserved (Sarvam wins if quality is close — Whisper loses on residency). (3) For ramp languages, the same eval must pass before adding each — ship without a language rather than ship low-quality voice. (4) **Self-hosted AI4Bharat OSS models retained at module boundary as DR fallback** — if Sarvam has a sustained outage, the same model lineage can be served from our own Modal/Fly deployment as a Plan-B (Op-ex spike temporarily acceptable in DR mode). (5) Eval gates AUR-2 brief promotion — Architect re-reads the result before AUR-2 ships.
  - **Area (required, tag):** architecture / AI / UX.

- [2026-05-26] [Engineer / Vivek] **R-SPEECH-2: Vendor concentration + cost-floor risks with Sarvam-primary.** Sarvam (single private company) is now both the model source AND the platform operator — a Sarvam outage = full speech outage with no managed-provider fallback (the AI4Bharat OSS DR path exists but requires us to spin up self-hosted inference, ~hours of operator work, not minutes). Cost moves from negligible (Bhashini free tier) to a real per-session line item (rough public estimate: $0.005–0.015/min ASR; similar TTS → ~₹4-12 per 5-min session). At 8-10 sessions / WAR / month this still fits under the ≤₹20/WAR/mo Cost fitness ceiling with single-digit-rupee headroom, but the cost floor is no longer zero.
  - **Likelihood (required):** low for major outage (Sarvam is well-funded, Lightspeed-led Series A 2024); medium for short blips; **high for "cost actuals exceed back-of-envelope estimate"** until first month of real usage data lands.
  - **Impact (required):** high if outage (voice = primary UX in AUR-2); medium if cost actuals are 2× estimate (still under the WAR ceiling at 2×; > 3× would force amendment).
  - **Mitigation (required):** (1) Self-hosted AI4Bharat OSS DR runbook drafted before AUR-2 ships — known recipe to bring Plan-B online inside 1 hour. (2) Cost telemetry per Sarvam API call piped into Vercel Observability from Day 1 of AUR-2; alert if rolling-7-day average exceeds ₹15/WAR/mo (75% of the ceiling). (3) Sarvam contract / SLA reviewed at AUR-2 promotion — if no SLA on offer, factor that into the eval. (4) Architecture v3 amendment triggered if either DR is invoked twice in any rolling 30-day window OR cost exceeds 75% of WAR ceiling on a sustained basis.
  - **Area (required, tag):** architecture / AI / vendor / cost.
