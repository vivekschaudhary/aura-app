...
# Foundational Product Bet — Aura (v2)
...
**Amendments from v1 (superseded 2026-05-24):**
1. **Pricing:** ship free at the user level in v1; defer the pricing decision until after 100,000 WAR is reached. Vision aspirational floor ("less than the price of a chai") remains intact for if/when a paid model is introduced.
2. **Cost guardrail clarified:** the ≤ ₹20/month figure was always the **architectural cost ceiling** (what Aura spends per active user), not a user-facing price. Now stated separately under Guardrails so it cannot drift.

---

## Vision

Aura is the friend the 536 million underserved Indians never had — a patient, non-judgemental, vernacular AI counsel that helps a person hear themselves clearly when they face decisions about career, relationships, money, and meaning. Aura does not advise. It asks the right question, in the language the user thinks in. In v1, Aura is free at the user level so cost is never a barrier to clarity. If we succeed, clarity stops being a privilege.
...
| Axis | Aura (v1) | Western AI companions (Replika, Pi) | Indian wellness apps (Wysa, YourDost) | Astrology apps (Astrotalk) | Tele-MANAS |
|------|-----------|--------------------------------------|----------------------------------------|------------------------------|------------|
| Language | Vernacular-first | English-first | English-default | Vernacular | Vernacular |
| User price | **Free in v1; pricing decision deferred to post-100K WAR** | ~$10–20/mo | ~₹500/mo (Wysa Premium) | Per-minute paid calls | Free |
...
**The contested neighbour is Astrotalk** — ₹1,182 cr revenue FY25, ~35M users, the *actual* product Aura's users use today when they want guidance on a decision. Aura competes here on substance (reasoned reflection) rather than substituting one form of belief for another. Free-at-user-level removes the price barrier Astrotalk imposes (per-minute calls add up fast).
...
- *Free model makes this metric purer.* No subscription paywall confounds engagement signal — pure product-fit measurement.

**Year-1 target:** 100,000 WAR. (Astrotalk reached 35M MAU over multiple years; Wysa has 1M+. 100K WAR is ~ Wysa's order-of-magnitude in year 1, with deeper engagement implied — and easier to reach without a paywall in front of the funnel.)
...
- **Objective 3:** Hold architectural cost discipline so the free-in-v1 model is funded sustainably.
  - KR 1: All-in cost per WAR per month ≤ ₹20 at 1,000+ WAR sustained.
  - KR 2: Monthly burn-vs-funding-runway review tracked in DRI log; no surprise overruns.
  - KR 3: At 100K WAR, documented pricing-decision recommendation produced (one of: stay free + sustain on funding/grants; introduce optional support/donation; introduce paid tier with free baseline preserved; introduce paid-only).
...
- **Objective:** Stand up the product foundation that the year-1 bet depends on.
  ...
  - KR 4: Funding plan for the 0→100K WAR free-burn phase committed in writing (founder capital, grant, seed round, etc.).

## Hypothesis (the bet)

> If we build a vernacular, mobile-first, **free-to-the-user** AI counsel grounded in reflective questioning and persistent memory of the user's story, then underserved Indians (Tier 2/3, vernacular-first, currently un-mentored) will return to it weekly to think through real decisions — measured by **100,000 Weekly Active Reflectors with D30 retention ≥ 25% and clarity score ≥ 4.0/5.0 within 12 months of launch**, **while holding architectural cost ≤ ₹20 / WAR / month so the free model is sustainable.** Pricing decision deferred to post-100K-WAR review (Objective 3 KR3).
...
| Data / proprietary intelligence | yes (primary) | Vernacular emotional-decision conversations at Indian-cultural scale = a training corpus no Western LLM has and no Indian incumbent (Astrotalk's astrology call data is a different domain) aggregates. Compounds as WAR grows. **Free model accelerates corpus accumulation.** |
...
| Brand / trust | yes (primary) | In emotional counsel, trust *is* the product. Aura's promise — non-judgemental, vernacular, **free in v1**, doesn't sell to you — is a deliberate, slow-to-copy positioning play. Brand here compounds with switching costs (you trust the friend who already knows you). |
...
- **User-facing price (v1):** **Free.** No paywall, no required subscription, no required donation. Optional support paths may be introduced but never block usage.
- **Architectural cost ceiling (the burn-rate discipline):** Median **architectural cost / WAR / month ≤ ₹20** (infra + AI inference + storage). This is what Aura spends per active user — not what a user pays. If unit economics force this above ₹20, the bet has failed at the architecture level even if WAR hits target. (Mirrored as the Cost fitness function in the foundational architecture bet.)
...
- **Free at user level for the entire v1 measurement window** (until 100K WAR reached or explicitly amended).
...
- **Ad-supported model in v1.** Closes the door on the easy free-monetization path; protects the trust moat.
...
- [2026-05-24] [PM] **AMENDMENT (supersedes nothing prior — new scope decision).** Ship free at user level in v1; defer the user-facing pricing decision until post-100K WAR.
  - **Rationale (required):** Per user direction (Vivek, 2026-05-24). Free removes the price variable from the WAR measurement (purer product-fit signal), accelerates the data moat (more conversations sooner), and is vision-aligned ("clarity is not a luxury"). The architectural cost discipline (≤₹20/WAR/mo) is preserved as burn-rate accountability — independent of user price. Pricing decision becomes a separate, evidence-based call at 100K WAR (Objective 3 KR3 added).
  - **Area (required, tag):** product / pricing / strategy.
  - **Alternatives considered (required):** Path B — drop architectural cost ceiling entirely (rejected: removes accountability; opens door to expensive engineering choices that are hard to walk back). Path C — free with usage caps (rejected for v1: caps add friction and policy surface area for marginal burn protection given the architecture is already cost-disciplined; preserved as an option if burn outpaces plan).
  - **Reversibility:** medium. Charging post-100K WAR is a hard product move (free-to-paid conversions are difficult), but the alternative of staying free is also acceptable. Optional support / donation paths preserve flexibility.

- [2026-05-24] [PM] **AMENDMENT.** Clarified that the ≤₹20 figure in the v1 Guardrails was always the architectural cost ceiling, not a user-facing price. v2 Guardrails section makes this explicit and separates the two.
  - **Rationale (required):** v1 wording ("Median user cost / month ≤ ₹20") was ambiguous between user-facing price and architectural cost. Architecture bet had already interpreted it as architectural cost. v2 makes the architectural-cost interpretation canonical.
  - **Area (required, tag):** product / clarification.
  - **Alternatives considered (required):** Leave wording ambiguous (rejected: invites drift). Interpret it as user price and lower the architectural ceiling (rejected: architecture has already been designed around ₹20 unit cost).
  - **Reversibility:** easy.
...
- [2026-05-23] [PM] R1 — Persona validation gap: vision's "informal-sector Indian" persona is not validated by current research.
  ...
  - **Mitigation (required):** Q3 OKR KR3 commissions 10–20 qualitative interviews before foundational architecture freezes. If field work invalidates the informal-sector persona, return to foundational product bet for amendment (creates v3).
...
- [2026-05-23] [PM] R3 — Unit economics at sub-₹20/month require open-source-stack inference cost discipline; falling back to GPT-4-class API per turn would break the price thesis.
  ...
  - **Mitigation (required):** Constraint flows to foundational architecture bet — explicit requirement to design on AI4Bharat / Bhashini / Sarvam stack with cost ceiling per WAU. Architect to size budget per turn before committing to stack. **(Reaffirmed in v2 — even more load-bearing now that no user revenue offsets the burn.)**
...
- [2026-05-24] [PM] **R6 — Free-with-deferred-pricing burn risk (NEW with v2).** Without user revenue, the 0→100K WAR phase is funded entirely by founder capital + investors + grants. Burn ≈ ₹2.4 cr/yr at 100K WAR if architectural cost holds at ₹20/WAR/mo — substantially more during the ramp.
  - **Likelihood (required):** high (will happen by definition).
  - **Impact (required):** high (existential if unfunded; no fallback if cost discipline slips).
  - **Mitigation (required):** Q3 OKR KR4 requires a written funding plan before architecture freeze. Monthly burn vs runway review in DRI log (Objective 3 KR2). Architectural cost ceiling (₹20/WAR/mo) becomes non-negotiable, not aspirational — it is the *only* cost discipline since there is no revenue.
  - **Area (required, tag):** product / economics / funding.

- [2026-05-24] [PM] **R7 — Free-to-paid conversion risk at 100K WAR (NEW with v2).** Behavioral economics: introducing a paywall after free is one of the hardest product moves. User base optimized for free may have lower willingness-to-pay than user base optimized for paid-from-start.
  - **Likelihood (required):** medium (will be in play only if we decide to charge at 100K).
  - **Impact (required):** medium-high (brand cost on transition; loss of users who can't afford the new tier).
  - **Mitigation (required):** Telegraph the deferral honestly in the product itself ("Aura is free in v1; we'll let you know if anything changes") so the eventual change is not a betrayal. At 100K WAR, prefer optional-support or freemium-with-free-baseline-preserved over forcing all users to pay (Objective 3 KR3 options).
  - **Area (required, tag):** product / pricing / brand.
...
- [2026-05-24] [PM] **Funding plan for the 0→100K WAR free-burn phase not yet committed in writing (NEW with v2).** Required by Q3 OKR KR4.
  - **Severity (required, mandatory):** P1 (blocks safe launch — running free without a funding plan invites R6 materialization).
  - **Owner (required, mandatory):** PM (with Vivek as decision owner).
  - **Status:** open.
  - **Area (required, tag):** product / funding.
